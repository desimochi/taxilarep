...
import StudentAttendanceTable from "./StudenTable";
...
export default function AttendanceFilter() {
  ...
  const applyFilter = async () => {
    ...
    const url = `all-student-all-subject-attenance?course=${course}&batch=${batch}&term=${term}&specialization=${specialization}`;
  ...
  return (
    <div className="p-6 bg-white rounded-xl shadow space-y-6">
      ...
      {/* ===== RESULT PREVIEW ===== */}
      {result && (
        <StudentAttendanceTable data = {result} />
      )}
    </div>
  );
}
