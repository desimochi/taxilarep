...
export default function Page(){
       const[message, setMessage] = useState("")
        ...
        useEffect(()=>{
           async function fetchDepartmentData(){
            setLoading(true)
                    try {
                        const respose = await authFetch(`role-viewset`)
        ...
        const handleEditClick = async (classId, type) => {
            try {
                const response = await authFetch(`role-viewset/${classId}`);
       ...
       async function handleAddDepartment(){
                try {
                            const response  = await authFetch(`role-viewset`, {
                                method: "POST",
                                headers: {
                            ...
                            setMessage("Add Role Succcessfully")
                            setShowToast(true)
                            setTimeout(()=>{
                                setShowToast(false)
        ...
        async function handleEditSaveDepartment(id){
            try {
                        const response  = await authFetch(`role-viewset/${id}`, {
                            method: "PUT",
                            headers: {
                                "Content-Type": "application/json",
                            },
                        ...
                        setMessage("Edit Role Succcessfully")
                        setShowToast(true)
                        setTimeout(()=>{
                            setShowToast(false)
                            window.location.reload()
    ...
    />
          ...
          <section className="relative">
           ...
           <div className="py-4 px-5">
            ...
            <h1 className="text-3xl font-bold mb-2 font-sans">Roles Details </h1>
            <p className="text-sm text-gray-500 mb-8">See, Create and Edit the Roles for Taxila Business School</p>
