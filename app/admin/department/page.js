...
export default function Page(){
       const[message, setMessage] = useState("")
        ...
        const filteredExams = department.filter((exam) => {
        ...
        useEffect(()=>{
           async function fetchDepartmentData(){
            setLoading(true)
                    try {
                        const respose = await authFetch(`department-viewset`)
        ...
        const handleEditClick = async (classId, type) => {
            try {
                const response = await authFetch(`department-viewset/${classId}`);
       ...
       async function handleAddDepartment(){
                try {
                            const response  = await authFetch(`department-viewset`, {
                                method: "POST",
                                headers: {
                            ...
                            setMessage("Add Department Succcessfully")
                            setShowToast(true)
                            setTimeout(()=>{
                                setShowToast(false)
        ...
        async function handleEditSaveDepartment(id){
            try {
                        const response  = await authFetch(`department-viewset/${id}`, {
                            method: "PUT",
                            headers: {
                                "Content-Type": "application/json",
                            },
                        ...
                        setMessage("Edit Department Succcessfully")
                        setShowToast(true)
                        setTimeout(()=>{
                            setShowToast(false)
                            window.location.reload()
    ...
    />
          ...
          <section className="relative">
           ...
           <div className="py-4 px-5">
            ...
            <h1 className="text-3xl font-bold mb-2 font-sans">Department Details </h1>
            <p className="text-sm text-gray-500 mb-8">Know About the Departments of Taxila Business School</p>
  ...
  onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search Department...." className="p-2 rounded-sm border border-x-gray-200" />
                    <button onClick={() => handleAddClick("add")} className="flex justify-center items-center border border-gray-200 hover:bg-gray-200 px-4"> <PlusIcon className="h-4 w-4"/>Add Department</button>
                </div>
...
</tbody>) : <tbody><tr> <td className="P-3">No Department Data Found </td></tr> </tbody>}
            </table>
            </div>
            </div>
            </div>
