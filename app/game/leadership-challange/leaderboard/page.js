...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    async function fetchLeaderboard() {
      try {
        const res = await fetch("/api/game/leader/leaderboard");
...
function formatTime(seconds) {
  ...
  return (
    <div className="p-8 min-h-screen bg-black">
      ...
      <h1 className="text-3xl font-bold mb-6 text-center text-white">Leadership Challange Leaderboard</h1>
        ...
        <table className="min-w-full shadow-lg ">
          <thead>
            <tr className="bg-white/30 backdrop-blur-sm text-yellow-300 border-b-1 border-yellow-100 ">
              ...
              <th className="p-3 ">Brand Reputation</th>
