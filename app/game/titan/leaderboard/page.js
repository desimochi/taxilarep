...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    async function fetchLeaderboard() {
      try {
        const res = await fetch("/api/game/titan/leaderbaord");
        const data = await res.json();
        setLeaderboard(data.leaderboard || []); // your API returns an array directly
      } catch (err) {
          ...
          <thead>
            <tr className="bg-white/30 backdrop-blur-sm text-yellow-300 border-b border-yellow-100">
              ...
              <th className="p-3">Stock Price</th>
               <th className="p-3">TSR</th>
               <th className="p-3">Valuation Multiple</th>
            ...
            {leaderboard.length > 0 ? (
              leaderboard.map((row, i) => (
                ...
                >
                  ...
                  <td className="p-3">{row?.stockPrice.toFixed(2) ?? "-"}</td>
                  <td className="p-3">{row?.tsr ?? "-"}</td>
                  <td className="p-3">{row?.valuationMultiple ?? "-"}</td>
