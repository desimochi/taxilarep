...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    async function fetchLeaderboard() {
      try {
        const res = await fetch("/api/game/lastcity/leaderboard");
...
function formatTime(seconds) {
  ...
  return (
    <div className="p-8 min-h-screen bg-black">
      ...
      <h1 className="text-3xl font-bold mb-6 text-center text-white">Lastcity Leaderboard</h1>
        ...
        <table className="min-w-full shadow-lg ">
          <thead>
            <tr className="bg-white/30 backdrop-blur-sm text-yellow-300 border-b-1 border-yellow-100 ">
              ...
              <th className="p-3 ">Discontent</th>
              <th className="p-3 ">Hope</th>
              <th className="p-3 ">Population</th>
              <th className="p-3 ">Cirtical Thinking Score (CTS)</th>
          ...
          <tbody>
            {leaderboard.length>0 ?  leaderboard.map((row, i) => (
              ...
              >
                ...
                <td className="p-3 ">{row.discontent}</td>
                <td className="p-3 ">{row.hope}</td>
                <td className="p-3 ">{row.population}</td>
                <td className="p-3 ">{row.criticalThinkingScore}</td>
