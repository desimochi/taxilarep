...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    async function fetchLeaderboard() {
      try {
        const res = await fetch("/api/game/samudra/leaderboard");
  ...
  return (
    <div className="p-8 min-h-screen bg-black">
      <h1 className="text-3xl font-bold mb-6 text-center text-white">Samundra Rakshak Leaderboard</h1>
          ...
          <tbody>
            {leaderboard.length>0 ?  leaderboard.map((row, i) => (
              ...
              >
                ...
                <td className="p-3 ">{row.gameState?.playerName || "Unknown"}</td>
                <td className="p-3 ">{row.gameState?.budget || "-"}</td>
                <td className="p-3 ">{row.gameState?.score?.toLocaleString()}</td>
                <td className="p-3 ">{row.gameState?.turn?.toLocaleString()}</td>
