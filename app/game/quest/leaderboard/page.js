...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    async function fetchLeaderboard() {
      try {
        const res = await fetch("/api/game/quest/leaderbaord");
...
function formatTime(seconds) {
  ...
  return (
    <div className="p-8 min-h-screen bg-black">
      ...
      <h1 className="text-3xl font-bold mb-6 text-center text-white">Taxila Capital Quest Leaderboard</h1>
        ...
        <table className="min-w-full shadow-lg ">
          <thead>
            <tr className="bg-white/30 backdrop-blur-sm text-yellow-300 border-b-1 border-yellow-100 ">
              ...
              <th className="p-3 ">Company Value</th>
              <th className="p-3 ">Cash</th>
              <th className="p-3 ">CEO Year</th>
          ...
          <tbody>
            {leaderboard.length>0 ?  leaderboard.map((row, i) => (
              ...
              >
                ...
                <td className="p-3 ">{row.companyValue || "-"}</td>
                <td className="p-3 ">{row.cash}</td>
                <td className="p-3 ">{row.year}/10</td>
