...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    async function fetchLeaderboard() {
      try {
        const res = await fetch("/api/game/interview/leaderboard");
...
function formatTime(seconds) {
  ...
  return (
    <div className="p-8 min-h-screen bg-white">
      ...
      <h1 className="text-2xl font-bold mb-6 text-gray-900 text-center ">Interview Leaderboard</h1>
        ...
        <table className="min-w-full shadow-lg ">
          ...
          <tbody>
            {leaderboard.length>0 ?  leaderboard.map((row, i) => (
              <tr
                key={row.userId}
                className={`bg-white/10 backdrop-blur-sm text-gray-700 text-sm text-center`}
              >
                <td className="p-3  font-bold">#{i + 1}</td>
                <td className="p-3 ">{row?.playerName || "Unknown"}</td>
                <td className="p-3 ">{row.highestScore || "-"}</td>
