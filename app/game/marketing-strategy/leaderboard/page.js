...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    async function fetchLeaderboard() {
      try {
        const res = await fetch("/api/game/marketing/leaderboard");
...
function formatTime(seconds) {
  ...
  return (
    <div className="p-8 min-h-screen bg-black">
      ...
      <h1 className="text-3xl font-bold mb-6 text-center text-white">Marketing Strategy Leaderboard</h1>
        ...
        <table className="min-w-full shadow-lg ">
          <thead>
            <tr className="bg-white/30 backdrop-blur-sm text-yellow-300 border-b-1 border-yellow-100 ">
              ...
              <th className="p-3 ">Balance Scorecard</th>
              <th className="p-3 ">Latest Market Share</th>
              <th className="p-3 ">Latest Net Profit</th>
          ...
          <tbody>
            {leaderboard.length>0 ?  leaderboard.map((row, i) => (
              ...
              >
                ...
                <td className="p-3 ">{row.latestBalancedScorecard}</td>
                <td className="p-3 ">{row.latestMarketShare.toFixed(2)}</td>
                <td className="p-3 ">{row.latestNetProfit.toFixed(2)}</td>
