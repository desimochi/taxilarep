...
const handleSubmit = async () => {
    console.log(state, selectedFaculty.id, newStatus )
        try {
            const response  = await authFetch(`taxila-currency-transfer`, {
                method: "POST",
                headers: {
                ...
                <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center">
                  {showtoast && <Toast message={message}/>}
                    <div className="bg-white p-6 rounded-lg shadow-lg w-96">
                        ...
                        <label htmlFor="amount" className="text-sm font-bold">Enter Amount</label>
                        <input type="number " placeholder="enter amount to transfer" className="border p-2 w-full rounded mb-4"  value={newStatus} onChange={(e)=>setNewStatus(e.target.value)}/>
                ...
                {filteredStudents.slice(0, pageSize).map((student, index) => (
                  <tr key={student.id} className="bg-white border-b hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 flex gap-2 ">
                      <SendIcon className="h-5 w-5 cursor-pointer" onClick={() => handleEditClick(student)}/>
                    </td>
                  </tr>
                ))}
              </tbody>
