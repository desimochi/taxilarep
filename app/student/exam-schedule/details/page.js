    ...
    const fetchclassData = async () => {
      try {
        ...
        const [response] = await Promise.all([
            await authFetch(`exam-subject-wise/${stuId}`),
...
function handleModal(exam){
  ...
  return (<>{details && selectedExam && <div
    id="crud-modal"
    ...
    <div className="relative p-4 w-full max-w-5xl max-h-full">
        <div className="relative bg-white rounded-xl shadow-sm dark:bg-gray-700">
            <div className="flex items-center justify-between p-6 md:p-5 border-b rounded-t dark:border-gray-600 border-gray-200">
                ...
                <button onClick={() => setDetails(false)} className='bg-red-600  p-2 rounded-sm text-white'>
                    <CrossIcon className='h-6 w-6 rotate-45'/>
                </button>
            </div>
            <div className='p-4'>
                <div dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(selectedExam.component?.description || '') }} />
            </div>
        </div>
    </div>
</div>}
                    ...
                    <tbody>
                        {examData.map((exam, index) => (
                            <tr key={exam.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td className="px-4 py-2 flex gap-2 text-blue-600" onClick={() => handleModal(exam)}><EyeIcon className='h-5 w-5'/ >See Details</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
