...
export default function Temp({id}) {
...
<div className="flex gap-2 items-center">
  {see==='internal' && allInternalComponentNames.length > 0 && (
          <div
        ...
        {/* External Filter */}
        {see==='external' &&  allExternalComponentNames.length > 0 && (
          <div
            className="mb-6 relative w-full max-w-md mt-6"
 ...
 <button className={`${see==='total'? 'bg-red-800 text-red-50 rounded-sm':''} p-2`} onClick={()=>setSee('total')}>See Total</button>
      ...
      {data.length > 0 && allExternalComponentNames.length > 0 && (
        ...
        <div className="overflow-x-auto px-12 mb-10">
          ...
          <table className="text-sm text-center border border-red-300 w-full">
            <thead>
              <tr className="text-red-800 bg-red-50 font-normal">
                <th className="px-6 py-3 whitespace-nowrap">S.No.</th>
                <th className="px-6 py-3 whitespace-nowrap">Student Name</th>
    ...
    <table className="text-sm text-center border border-red-300 w-full">
      <thead>
        <tr className="text-red-800 bg-red-50 font-normal">
          <th className="px-6 py-3 whitespace-nowrap">S.No.</th>
          <th className="px-6 py-3 whitespace-nowrap">Student Name</th>
          <th className="px-6 py-3 whitespace-nowrap">Internal Total</th>
          <th className="px-6 py-3 whitespace-nowrap">External Total</th>
          <th className="px-6 py-3 whitespace-nowrap">Grand Total</th>
        </tr>
      </thead>
      <tbody>
        {data.map((student, idx) => {
          ...
          const internalMarks = student.component_marks.filter(m => m.component_type === 'INTERNAL');
          const externalMarks = student.component_marks.filter(m => m.component_type === 'EXTERNAL');

          const internalTotal = internalMarks.reduce((sum, mark) => sum + (mark.obtained_marks || 0), 0);
          const internalMax = internalMarks.reduce((sum, mark) => sum + (mark.max_marks || 0), 0);

          const externalTotal = externalMarks.reduce((sum, mark) => sum + (mark.obtained_marks || 0), 0);
          const externalMax = externalMarks.reduce((sum, mark) => sum + (mark.max_marks || 0), 0);

          const grandTotal = internalTotal + externalTotal;
          const grandMax = internalMax + externalMax;

          return (
            <tr key={student.id} className="border-b">
              <td className="px-6 py-3">{idx + 1}</td>
              <td className="px-6 py-3">{fullName}</td>
              <td className={`px-6 py-3 ${internalTotal < internalMax/2 ? "text-red-600" : ""}`}>
                {internalTotal} / {internalMax}
              </td>
              <td className={`px-6 py-3 ${externalTotal < externalMax/2 ? "text-red-600" : ""}`}>
                {externalTotal} / {externalMax}
              </td>
              <td className={`px-6 py-3 font-semibold ${grandTotal < grandMax/2 ? "text-red-600" : ""}`}>
                {grandTotal} / {grandMax}
              </td>
            </tr>
          );
