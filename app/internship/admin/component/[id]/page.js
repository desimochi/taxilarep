...
if(!batch){
  return <div>Please select Batch to view assigned students</div>
}
    useEffect(() => {
    ...
    <h1 className="text-3xl font-bold mb-2 font-sans">Internship Assigned Students </h1>
                        ...
                        filteredMentees.map((stu, index) => (
                            <tr key={stu.id} className="border-b text-sm">
                                ...
                                <td className="px-6 py-4 flex items-center justify-center">
                                   <Link href={`/internship/mentor/report/${stu.id}?compId=${id}`} className="font-medium text-green-800 bg-green-100 px-2 py-0.5 rounded-sm text-xs border border-green-200 hover:underline flex items-center w-fit">
