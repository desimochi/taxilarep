...
export default function Page() {
    ...
    useEffect(() => {
        const fetchClassData = async () => {
            try {
                setLoading(true)
                const response = await authFetch(`hall-ticket-viewset`)
                                        ...
                                        {filteredClasses.length > 0 ? (
                                            filteredClasses.map((cls, index) => (
                                                <tr key={cls.id} className="border-b text-sm hover:bg-gray-50">
                                                    ...
                                                    <td className="px-6 py-3">{cls.type ==="main"? "main" : `${cls.type}`}</td>
                                                    <td className="px-6 py-3">{cls.is_active ? "Active" : "Inactive"}</td>
                                                    <td className="px-6 py-3 flex justify-center">
                                                        <Link href={`/exam-components/admitcard/edit?id=${cls.id}`}>
                                            ...
                                            <tr>
                                                <td colSpan={5} className="text-center py-4">No Admit Cards Announcement Found</td>
                                            </tr>
                                        )}
                                    </tbody>
                                </table>
